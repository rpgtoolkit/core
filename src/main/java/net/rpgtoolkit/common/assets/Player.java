...
public class Player extends BasicType {
  ...
  public void setHasSpecialMoves(boolean hasSpecialMoves) {
    if (hasSpecialMoves == true) {
      this.hasSpecialMoves = 1;
    } else {
      this.hasSpecialMoves = 0;
    }
  }
  ...
  public Area getCollisionArea(int correctX, int correctY, int shiftX, int shiftY) {
    ...
    for (Point point : baseVector.getPoints()) {
      collisionPoly.addPoint(point.x + correctX + (
              currentXLocation + shiftX), point.y + correctY + (currentYLocation + shiftY));
    }
